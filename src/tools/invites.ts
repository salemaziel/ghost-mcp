...
export function registerInviteTools(server: McpServer) {
  ...
  server.tool(
    ...
    async (args, _extra) => {
        ...
        try {
            ...
            const url = `${config.url}/ghost/api/admin/invites/`;
            const headers = {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify(response.data.invites, null, 2),
                    },
                ],
            };
        } catch (error: any) {
            ...
            return {
                ...
                content: [
                    {
                        type: "text",
                        text: `invites_browse failed. status=${status}\n${bodyText}`,
                    },
                ],
            };
        }
  ...
  server.tool(
    ...
    async (args, _extra) => {
        ...
        try {
            ...
            const response = await axios.post(url, { invites: [args] }, { headers });
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify(response.data.invites[0], null, 2),
                    },
                ],
            };
        } catch (error: any) {
            ...
            return {
                ...
                content: [
                    {
                        type: "text",
                        text: `invites_add failed. status=${status}\n${bodyText}`,
                    },
                ],
            };
        }
  ...
  server.tool(
    ...
    async (args, _extra) => {
        ...
        try {
            const token = generateGhostAdminToken(config.key);
            const url = `${config.url}/ghost/api/admin/invites/${args.id}/`;
            const headers = {
                'Authorization': `Ghost ${token}`
            };
            ...
            return {
                ...
                content: [
                    {
                        type: "text",
                        text: `invites_delete failed. status=${status}\n${bodyText}`,
                    },
                ],
            };
        }
